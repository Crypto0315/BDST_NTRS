...
public class BlockChain {
    ...
    public Transaction sendTransaction(Transaction transaction, String privateKey) throws Exception {
        ...
        transaction.setNtrsStep(stringObjectMap);
         ...
         */
        Object[] ntrsSign = SignUtils.ntrsSign(stringObjectMap, sender.get().getPublicKey(), privateKey);
        transaction.setNtrsSign(ntrsSign);
        ...
        //先验证私钥是否正确
		/*if (!SignUtils.verify((byte[]) sender.get().getPublicKey(), sign, transaction.toString())) {
         ...
         * 验证签名
         */
        String result = SignUtils.ntrsVerify(stringObjectMap,ntrsSign);
        if (!"1".equals(result)){
